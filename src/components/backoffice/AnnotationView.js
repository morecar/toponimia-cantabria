...
// ── Main component ────────────────────────────────────────────────────────────
export default function AnnotationView({ activeProj, repository, onBack, onEditProject }) {
  const { drafts, saveDraft, newDraftId } = useDraftStore()
  ...
  const addManualAttestation = () => {
    ...
    const att = {
      ...
      url:         activeProj.url || '',
      projectId:   activeProj.id,
      occurrences: [{ highlight: manualSelection, quote: manualSelection }],
    }
    ...
    <div className="bo-annotate-layout">
      ...
      <div className="bo-annotate-text">
        <div className="bo-annotate-text-toolbar">
          <button className="bo-btn bo-btn-sm" onClick={onBack}>← Textos</button>
          <span className="bo-annotate-proj-name">
            {activeProj.title}{activeProj.year ? ` (${activeProj.year})` : ''}
          </span>
          <button className="bo-btn bo-btn-sm" onClick={() => onEditProject(activeProj)}>Editar texto</button>
        </div>
        <HighlightedText
                  ...
                  {linkedDrafts.map(d => (
                    <div key={d.draftId || d.hash} className="bo-linked-item">
                      ...
                      {d.projectAtts.map((att, ai) => (
                        att.occurrences.map((occ, oi) => occ.quote && (
